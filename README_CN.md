...
5. **核查** — 与 LLM 并行的确定性法条核查层，专查那些答错就是违法而非观点分歧的硬线。核查层按法域区分（默认 `--jurisdiction auto` 自动识别，也可用 `--jurisdiction cn` / `--jurisdiction us-ca` 强制指定）：中文合同核查试用期时长与工资上限、违约金适用范围、竞业限制期限与补偿、加班费 150%/200%/300% 下限（《劳动法》《劳动合同法》）、定金比例与租赁期限上限（《民法典》）、民间借贷利率司法保护上限（签约时一年期 LPR 四倍，最高法民间借贷司法解释第 25 条）与禁止预扣利息（《民法典》第 670 条砍头息规则）；锚定加州的住宅租约核查加州民法典规则：押金不得超过一个月租金且必须可退（§1950.5，经 AB 12 修订）、房东进屋须提前 24 小时通知（§1954）、退租后 21 天内须退还押金并附扣款明细（§1950.5(g)）。每条给出 违规/合规/无法判断 三种结论，条款缺失或法域无法确定时如实报「无法判断」而不是当作合规放过。可用 `--no-checklist` 关闭。

6. **报告** — 输出 Rich 格式的终端美化报告，或导出 Markdown/JSON/HTML 以便分享或进一步处理。
...
**已完成**：批量扫描（一次分析多份合同）、合同对比（diff 两个版本并逐条标出变化）、逐条协商意见稿（每个风险点可附带建议替换措辞，能直接粘进协商邮件）、法域感知的确定性法条核查（中国劳动法与《民法典》，外加美国加州住宅租赁规则，目前覆盖一个州）、Gradio Web 界面（`contractguard web`，LLM 分析和确定性法条核查面板同屏展示）。报告可导出排版干净的 PDF（含法条引用、官方链接和风险等级）：`--output report.pdf` 或 Web 端下载按钮；装了 weasyprint 系统库就走高保真渲染，没装也有内置纯 Python 渲染器兜底，任何机器都能导出。发版回归语料集（`corpus/`，由 `tests/test_corpus.py` 把门）在全长样本合同上钉死法条核查判定，规则和解析器改动不再能悄悄改变真实结果。
