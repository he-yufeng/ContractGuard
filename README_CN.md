...
5. **核查** — 与 LLM 并行的确定性法条核查层，专查那些答错就是违法而非观点分歧的硬线：试用期时长与工资上限、违约金适用范围、竞业限制期限与补偿、加班费 150%/200%/300% 下限（《劳动法》《劳动合同法》），定金比例与租赁期限上限（《民法典》）。每条给出 违规/合规/无法判断 三种结论，条款缺失时如实报「无法判断」而不是当作合规放过。可用 `--no-checklist` 关闭。
...
**已完成**：批量扫描（一次分析多份合同）、合同对比（diff 两个版本并逐条标出变化）、逐条协商意见稿（每个风险点可附带建议替换措辞，能直接粘进协商邮件）。
