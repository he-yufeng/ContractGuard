...
5. **核查** — 与 LLM 并行的确定性法条核查层，专查那些答错就是违法而非观点分歧的硬线。核查层按法域区分（默认 `--jurisdiction auto` 自动识别，也可用 `--jurisdiction cn` / `--jurisdiction us-ca` 强制指定）：中文合同核查试用期时长与工资上限、违约金适用范围、竞业限制期限与补偿、加班费 150%/200%/300% 下限（《劳动法》《劳动合同法》）、定金比例与租赁期限上限（《民法典》）；锚定加州的住宅租约核查加州民法典规则：押金不得超过一个月租金且必须可退（§1950.5，经 AB 12 修订）、房东进屋须提前 24 小时通知（§1954）、退租后 21 天内须退还押金并附扣款明细（§1950.5(g)）。每条给出 违规/合规/无法判断 三种结论，条款缺失或法域无法确定时如实报「无法判断」而不是当作合规放过。可用 `--no-checklist` 关闭。
